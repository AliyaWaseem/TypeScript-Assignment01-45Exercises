...
let current_users: string[] = ["AdmIn","WaseeM","Ahmed","HaSsan","ArHam","JoHn"];

//List of new users:
let new_users: string[] = ["Asif", "JoHn", "Hamzah", "ArHam", "Rizwan"];

//Loop through new_users to check for usernames availability:
new_users.forEach((newUser) => {
  let our_condition = current_users.some(
    (c_user) => c_user.toLowerCase() === newUser.toLowerCase()
  );
  if (our_condition) {
